...
class HostDevice {
public:
  void PushRequest(uint8_t opcode, uint32_t addr, uint32_t value);
...
void HostDevice::PushRequest(uint8_t opcode, uint32_t addr, uint32_t value) {
  HostRequest r;
  r.opcode = opcode;
  r.addr = addr;
  r.value = value;
...
void HostDevice::WaitPopResponse(HostResponse *r) { resp_.WaitPop(r); }
...
public:
  ...
  const char *type_key() const final { return "vta-tsim"; }
  ...
  GetFunction(const std::string &name,
    ...
    if (name == "WriteReg") {
      return TypedPackedFunc<void(int, int)>(
          [this](int addr, int value) { this->WriteReg(addr, value); });
    } else {
  ...
  void Init(const std::string &name) {
    Load(name);
    VTADPIInitFunc finit =
        reinterpret_cast<VTADPIInitFunc>(GetSymbol("VTADPIInit"));
    CHECK(finit != nullptr);
  ...
  void SimLaunch() {
    auto frun = [this]() { (*ftsim_)(); };
    tsim_thread_ = std::thread(frun);
  }

  void SimWait() { sim_device_.Wait(); }

  void SimResume() { sim_device_.Resume(); }
...
protected:
  ...
  void SimDPI(dpi8_t *wait, dpi8_t *exit) {
    *wait = sim_device_.GetWaitStatus();
    *exit = sim_device_.GetExitStatus();
  }

  void HostDPI(dpi8_t *req_valid, dpi8_t *req_opcode, dpi32_t *req_addr,
  ...
  static void VTASimDPI(VTAContextHandle self, dpi8_t *wait, dpi8_t *exit) {
    static_cast<DPIModule *>(self)->SimDPI(wait, exit);
  }

  static void VTAHostDPI(VTAContextHandle self, dpi8_t *req_valid,
                         dpi8_t *req_opcode, dpi32_t *req_addr,
                         dpi32_t *req_value, dpi8_t req_deq, dpi8_t resp_valid,
                         dpi32_t resp_value) {
    static_cast<DPIModule *>(self)->HostDPI(req_valid, req_opcode, req_addr,
                                            req_value, req_deq, resp_valid,
                                            resp_value);
  }
...
Module DPIModuleNode::Load(std::string dll_name) {
  std::shared_ptr<DPIModule> n = std::make_shared<DPIModule>();
  n->Init(dll_name);
  return Module(n);
}
