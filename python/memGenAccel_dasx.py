...
if platform.system() == 'Linux':
        hw_lib_path = "./python/build/libhw_dasx_{}_{}_{}_{}_{}_{}_{}.so".format(nw,ns,tbe,lock,nparal,nc,nword)
...
with open(sys.argv[2]) as trace:
    ...
    for (i,row) in enumerate(trigger):
        ...
        if(int(row['opcode']) == collect ):
            input_inst.append(collect)
            input_addr.append(int(row['address'])  )
            input_data.append(int(row['data']))
        elif(int(row['opcode']) == ack ):
            input_inst.append(ack)
            input_addr.append(0)
            input_data.append(int(row['address']))
