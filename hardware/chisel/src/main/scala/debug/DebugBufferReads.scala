...
class DebugBufferReads(numDebug: Int, val boreIDsList: Seq[Int])
  ...
  val read_debug_buffer_size = 64 // 8 * 5 bytes
  ...
  val read_buf_done = List.fill(numDebug)(Wire(Bool()))
  ...
  read_buf_done.foreach(_ := false.B)


  for(i <- 0 until numDebug){
    read_buffers(i).io.baddr := io.addrDebug(i)
    read_buffers(i).io.len := read_debug_buffer_size.U
    read_buffers(i).io.start := read_buf_start(i)
    io.vmeIn(i) <> read_buffers(i).io.ume_rd
    ...
    read_buf_done(i) := read_buffers(i).io.done
    ...
    BoringUtils.addSource(read_buf_done(i), s"in_log_Buffer_done${boreIDsList(i)}")
