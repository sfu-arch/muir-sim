...
class DandelionF1AccelDebug(accelModule: () => DandelionAccelDCRModule)(debugModule: () => DandelionAccelDebugModule)
  ...
  val shell = withClockAndReset(clock = ap_clk, reset = ~ap_rst_n) {
    Module(new DandelionF1DebugShell(accelModule)(debugModule)
    (numPtrs, numDbgs, numVals, numRets, numEvents, numCtrls))
  }
