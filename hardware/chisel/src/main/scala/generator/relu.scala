...
class reluDF(PtrsIn: Seq[Int] = List(64, 64), ValsIn: Seq[Int] = List(64), Returns: Seq[Int] = List())
  ...
  val phij_03212 = Module(new PhiFastNode(NumInputs = 2, NumOutputs = 2, ID = 12, Res = true, Debug = false, GuardVals=Seq.tabulate(100)(n => n)))

  //  %mul = mul i32 %j.032, %N, !UID !51
  val binaryOp_mul13 = Module(new ComputeNode(NumOuts = 1, ID = 13, opCode = "mul")(sign = false, Debug = false, GuardVals=GuardReader("relu.dbg"){13}))

  //  br label %for.body4, !dbg !52, !UID !53, !BB_UID !54
  val br_14 = Module(new UBranchNode(ID = 14))

  //  %inc13 = add nuw i32 %j.032, 1, !dbg !55, !UID !56
  val binaryOp_inc1316 = Module(new ComputeNode(NumOuts = 2, ID = 16, opCode = "add")(sign = false, Debug = false, GuardVals=GuardReader("relu.dbg"){16}))
