...
class DandelionSimStreamDebugAccel(accelModule: () => DandelionAccelDCRModule,
  ...
  val sim_wait = IO(Output(Bool()))
  val sim_shell = Module(new AXISimShell)
  val shell = Module(new DandelionStreamDebugShell(accelModule)(debugWriteModule)(debugReadModule)
  (numPtrs = numPtrs, numDbgs = numDbgs, numVals = numVals,
    numRets = numRets, numEvents = numEvents, numCtrls = numCtrls))
...
/**
  ...
  *        this is not a clean way to pass arguments
  */
object DandelionSimStreamDebugAccelMain extends App {
